  ...
  "node_modules/source-map-support/source-map-support.js"(exports2, module2) {
    ...
    function prepareStackTrace(error, stack2) {
      ...
      for (var i = stack2.length - 1; i >= 0; i--) {
        processedStack.push("\n    at " + wrapCallSite(stack2[i], state));
      ...
      const instance = (event, context, callback) => {
        ...
        const middyPromise = new Promise((resolve, reject) => {
          ...
          runMiddlewares(beforeMiddlewares, request, (err) => {
            ...
            const handlerReturnValue = handler2.call(request, request.event, request.context, (err2, response) => {
              if (err2)
...
var require_http_json_body_parser = __commonJS({
  "node_modules/@middy/http-json-body-parser/index.js"(exports2, module2) {
    ...
    module2.exports = (opts) => ({
      before: (handler2, next) => {
        opts = opts || {};
        if (handler2.event.headers) {
          const contentTypeHeader = handler2.event.headers["content-type"] || handler2.event.headers["Content-Type"];
          if (contentTypeHeader) {
            const { type } = contentType.parse(contentTypeHeader);
            if (type.match(/^application\/(.*\+)?json$/)) {
              try {
                const data = handler2.event.isBase64Encoded ? Buffer.from(handler2.event.body, "base64").toString() : handler2.event.body;
                handler2.event.body = JSON.parse(data, opts.reviver);
              } catch (err) {
...
// src/functions/postblog/handler.ts
__export(exports, {
  main: () => main
});
...
var middyfy = (handler2) => {
  return (0, import_core.default)(handler2).use((0, import_http_json_body_parser.default)());
};

// src/functions/postblog/handler.ts
var handler = async (event) => {
  const body = event.body;
  const blogBody = __spreadValues({
    recieved: true
  }, body);
